...
這個專案使用公開 API 產生可解釋的台股與 ETF 量化研究報告。
...
- 目標交易日以日期顯示，提供大盤上漲、平盤、下跌情境機率與反證條件
- 股票：趨勢、營收、估值、風險綜合評分
- ETF：趨勢、波動與流動性評分
- 每個研究名單標示研究理由、主要風險與資料不足處
- 同時輸出 Markdown 與 HTML

報告是依公開資料產生的一般性量化研究資訊，不考慮個人財務狀況，
也不保證未來績效，亦不構成買賣推介。
...
每日報告的「研究理由」優先使用公司重大訊息、月營收與產業事件。
...
「YYYY-MM-DD 推估」整合國際盤、期貨盤後盤、台股市場廣度、外資現貨、
外資台指期與匯率，輸出上漲／平盤／下跌三種機率。平盤定義為加權指數
收盤漲跌介於 ±0.3%，單一方向機率最高 60%；這是可檢查的情境猜測，
不預測點位，也不保證報酬。盤前報告使用當日交易日；若手動於收盤後產生，
則自動推往下一個台股開市日。
